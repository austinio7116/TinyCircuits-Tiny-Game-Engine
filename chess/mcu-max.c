...
// Configuration — MCUMAX_HASHING_ENABLED and MCUMAX_HASH_TABLE_SIZE
// are defined via build flags (micropython.cmake / micropython.mk)
...
/* Dynamically allocated — only uses memory when chess is active */
static struct HashEntry *mcumax_hash_table = NULL;
...
void mcumax_init()
{
    ...
    /* Hash table must be set via mcumax_set_hash_table() before init */
    if (mcumax_hash_table != NULL) {
        memset(mcumax_hash_table, 0,
               MCUMAX_HASH_TABLE_SIZE * sizeof(struct HashEntry));
    }
...
void mcumax_deinit(void)
{
#ifdef MCUMAX_HASHING_ENABLED
    /* Just clear the pointer — caller owns the memory */
    mcumax_hash_table = NULL;
#endif
}

#ifdef MCUMAX_HASHING_ENABLED
void mcumax_set_hash_table(void *table)
{
    mcumax_hash_table = (struct HashEntry *)table;
}

uint32_t mcumax_get_hash_table_entry_size(void)
{
    return sizeof(struct HashEntry);
}
#endif
