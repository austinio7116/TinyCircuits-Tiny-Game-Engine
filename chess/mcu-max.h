...
/**
 * @brief Frees dynamically allocated memory (hash table).
 * Call when done with the chess engine to release memory for other games.
 */
void mcumax_deinit(void);

/**
 * @brief Set the hash table buffer (caller-allocated).
 * Must be called before mcumax_init() when hashing is enabled.
 * @param table Pointer to buffer of MCUMAX_HASH_TABLE_SIZE * entry_size bytes.
 */
void mcumax_set_hash_table(void *table);

/**
 * @brief Get the size of one hash table entry in bytes.
 */
