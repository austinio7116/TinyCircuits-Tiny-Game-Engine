...
/* Thumby Color: longjmp target so I_Error doesn't kill the process */
jmp_buf doom_error_jmp;
int doom_error_active = 0;
char doom_error_msg[512];
...
#endif
#define DEFAULT_RAM 1  /* used as MiB fallback, overridden below */
#define MIN_RAM     1
...
static byte *AutoAllocMemory(int *size, int default_ram, int min_ram)
{
    ...
    /* Thumby device: allocate zone from GC heap */
    {
        extern void *m_malloc_maybe(size_t num_bytes);
        extern void gc_collect(void);
        gc_collect();  /* Reclaim all garbage first */
        int try_kb = DOOM_ZONE_KB;
        zonemem = NULL;
        while (try_kb >= 48 && !zonemem) {
            *size = try_kb * 1024;
            zonemem = m_malloc_maybe(*size);
            if (!zonemem) {
                gc_collect();  /* Try again after collect */
                zonemem = m_malloc_maybe(*size);
            }
            if (!zonemem) try_kb -= 4;
        }
        if (!zonemem) {
            I_Error("Zone: GC heap exhausted");
        }
        return zonemem;
    }
#elif defined(DOOM_THUMBY)
    /* Emulator with device-matching zone size for testing */
    *size = DOOM_ZONE_KB * 1024;
    zonemem = malloc(*size);
    if (!zonemem) I_Error("Zone: malloc failed");
    return zonemem;
#endif
...
void I_Error (char *error, ...)
{
    ...
    /* Thumby Color: longjmp back to caller instead of killing process */
    if (doom_error_active) {
        strncpy(doom_error_msg, msgbuf, sizeof(doom_error_msg) - 1);
        doom_error_msg[sizeof(doom_error_msg) - 1] = '\0';
        longjmp(doom_error_jmp, 1);
    }

    /* Fallback if no longjmp target set */
