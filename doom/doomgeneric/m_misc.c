...
{
#ifdef __arm__
    /* On ARM: check XIP flash WAD first, then MP file object */
    {
        #include <string.h>
        const uint8_t *xip = (const uint8_t *)(0x10000000 + 0x200000);
        if (memcmp(xip, "IWAD", 4) == 0 || memcmp(xip, "PWAD", 4) == 0)
            return true;
    }
    {
        extern mp_obj_t doom_wad_file_obj;
        if (doom_wad_file_obj != MP_OBJ_NULL) return true;
    }
    (void)filename;
    return false;
#else
