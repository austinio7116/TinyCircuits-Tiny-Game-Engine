...
// Z_Init
//
int doom_zone_size_kb = 0;  /* exported for debug */
...
void Z_Free (void* ptr)
{
    ...
    /* Check if pointer is within zone — if not, it's a calloc'd ptr, skip */
    if ((byte*)ptr < (byte*)mainzone ||
        (byte*)ptr >= (byte*)mainzone + mainzone->size) {
        return;  /* Not a zone pointer — allocated via calloc */
    }
#endif
  ...
  void*		user )
{
    ...
    do
    {
        if (rover == start)
        {
            /* Emergency: purge ALL purgeable blocks to coalesce free space */
            static int purge_count = 0;
            if (purge_count < 3) {  /* Limit retries to avoid infinite loop */
                purge_count++;
                memblock_t *b = mainzone->blocklist.next;
                int freed = 0;
                while (b != &mainzone->blocklist) {
                    memblock_t *next = b->next;
                    if (b->tag >= PU_PURGELEVEL) {
                        Z_Free((byte *)b + sizeof(memblock_t));
                        freed++;
                    }
                    b = next;
                }
                if (freed > 0) {
                    ...
                    base = mainzone->blocklist.next;
                    if (base->prev->tag == PU_FREE) base = base->prev;
                    rover = base;
                    start = base->prev;
                    purge_count = 0;  /* Reset since we freed something */
                    continue;
                }
            }
            purge_count = 0;
