...
/*  --- doc ---
    NAME: update_buttons
    ID: update_buttons
    DESC: Refresh button is_pressed/is_just_pressed/etc. state without running the full engine.tick() pipeline. Cheap (~10us). Intended for the ThumbyOne legacy `thumby` compat shim where we need to refresh button state on every pressed() poll without paying for an LCD DMA + framebuffer clear each time.
    RETURN: None
*/
